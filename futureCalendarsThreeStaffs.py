...
def format_staff( staff ):
    ...
    contexttools.KeySignatureMark('d', 'major')( staff[2] )
...
def format_lilypond_file(lilypond_file):
    lilypond_file.paper_block.paper_width = 14 * 25.4
    ...
    lilypond_file.paper_block.system_system_spacing = layouttools.make_spacing_vector(0, 0, 13, 0)
...
def make_future_calendars( melody, pitch_range_tuple):
    ...
    melody_doubler = componenttools.copy_components_and_covered_spanners(melody[:])
    melody.extend(melody_doubler)
    #format_melody(melody)
    voice = Voice([])
    voice.is_parallel = True
    voice.append(melody)
    second_voice = make_prolated_echo_voice( melody, 1 )
    third_voice = make_prolated_echo_voice( recitation, 2)
    for leaf in third_voice:
        leaf.written_pitch += 12
    melody_times_four = componenttools.copy_components_and_covered_spanners(melody[:])
    melody.extend(melody_times_four)
    second_voice.insert(0, Rest(Duration(3,4)))
    third_voice.insert(0, Rest(Duration(3,4)))
    third_voice.insert(0, Rest(Duration(3,4)))
    contexttools.DynamicMark('f')(recitation[0])
    contexttools.DynamicMark('f')(second_voice[1])
    contexttools.DynamicMark('f')(third_voice[2])
    staff[0].append(Voice(recitation))
    staff[0].append(voice)
    staff[1].extend(Rest("r2.")*16)
    staff[1].append(second_voice)
    staff[2].extend(Rest("r2.")*16)
    staff[2].append(third_voice)
    for line in staff:
        componenttools.split_components_at_offsets(line.leaves, [Duration(3,4)], cyclic=True)
    del(staff[2][-1][-2:])
    del(staff[1][-1][-1:])
    format_staff(staff)
